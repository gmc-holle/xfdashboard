 ...
 * g_object_unref().
 */
GdkPixbuf* _xfdashboard_application_icon_get_pixbuf_for_icon_name(const gchar *inIconName, const gchar *inFallbackIconName)
{
	GdkPixbuf		*icon=NULL;
	...
	if(!GARCON_IS_MENU(priv->menuElement) && !GARCON_IS_MENU_ITEM(priv->menuElement))
	{
		...
		else
		{
			...
			if(iconName)
			{
				...
				icon=_xfdashboard_application_icon_get_pixbuf_for_icon_name(iconName, GTK_STOCK_MISSING_IMAGE);
...
void _xfdashboard_application_icon_update_custom(XfdashboardApplicationIcon *self)
{
	...
	{
		/* Get icon for icon name */
		icon=_xfdashboard_application_icon_get_pixbuf_for_icon_name(priv->customIconName, GTK_STOCK_MISSING_IMAGE);
