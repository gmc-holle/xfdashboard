...
struct _XfdashboardViewpadPrivate
{
	...
	XfdashboardPolicy		horizontalScrollbarPolicy;

	gboolean				verticalScrollbarVisible;
	ClutterActor			*verticalScrollbar;
	XfdashboardPolicy		verticalScrollbarPolicy;
};
 ...
 */
static void xfdashboard_viewpad_pick(ClutterActor *self, const ClutterColor *inColor)
{
	...
	CLUTTER_ACTOR_CLASS(xfdashboard_viewpad_parent_class)->pick(self, inColor);

	/* Draw silouhette of active view and visible scroll bars */
	if(priv->activeView &&
		CLUTTER_ACTOR_IS_VISIBLE(priv->activeView))
	{
		clutter_actor_paint(CLUTTER_ACTOR(priv->activeView));
	}
	...
	if(priv->activeView)
	{
		...
		if(inForHeight>=0)
		{
			...
			clutter_actor_get_preferred_height(CLUTTER_ACTOR(priv->activeView),
												...
												NULL,
												&viewNaturalHeight);
			if(priv->horizontalScrollbarPolicy==XFDASHBOARD_POLICY_ALWAYS ||
				(priv->horizontalScrollbarPolicy==XFDASHBOARD_POLICY_AUTOMATIC &&
	...
	if(priv->activeView)
	{
		...
		if(inForWidth>=0)
		{
			...
			clutter_actor_get_preferred_width(CLUTTER_ACTOR(priv->activeView),
												...
												NULL,
												&viewNaturalWidth);
			if(priv->verticalScrollbarPolicy==XFDASHBOARD_POLICY_ALWAYS ||
				(priv->verticalScrollbarPolicy==XFDASHBOARD_POLICY_AUTOMATIC &&
