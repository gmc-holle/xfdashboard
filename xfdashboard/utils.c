...
GAppInfo* xfdashboard_garcon_menu_item_get_app_info(GarconMenuItem *inMenuItem)
{
	...
	error=NULL;
	appInfo=NULL;
	name=garcon_menu_item_get_name(inMenuItem);

	/* First try to create a desktop application info from desktop ID of menu element */
	desktopURI=garcon_menu_item_get_uri(inMenuItem);
	if(desktopURI)
	{
		...
		if(desktopFilename)
		{
			...
			return(G_APP_INFO(desktopAppInfo));
		}
	}

	/* If we get here creating desktop application info failed so fallback to create a simple application info */
	g_debug("Creating GDesktopAppInfo for menu item '%s' failed. Using fallback method!", name);

	command=garcon_menu_item_get_command(inMenuItem);

	flags=G_APP_INFO_CREATE_NONE;
	if(garcon_menu_item_supports_startup_notification(inMenuItem)) flags|=G_APP_INFO_CREATE_SUPPORTS_STARTUP_NOTIFICATION;
	if(garcon_menu_item_requires_terminal(inMenuItem)) flags|=G_APP_INFO_CREATE_NEEDS_TERMINAL;
